...
const sourceFile = "data/raw/us-census/imports/seafood/hs03-16-primary.csv";
...
const levels = [
  {
    ...
    codeLength: 2,
    codeField: "hsCode",
    outputFile: "seafood-imports-hs2.json",
    label: "HS2",
  },
  {
    id: "hs4",
    codeLength: 4,
    codeField: "hs4Code",
    outputFile: "seafood-imports-hs4.json",
    label: "HS4",
  },
  {
    id: "hs6",
    codeLength: 6,
    codeField: "hs6Code",
    outputFile: "seafood-imports-hs6.json",
    label: "HS6",
  },
  {
    id: "hs8",
    codeLength: 8,
    codeField: "hs8Code",
    outputFile: "seafood-imports-hs8.json",
    label: "HS8",
  },
];
...
async function readEntries() {
  ...
  const lines = text.split(/\r?\n/);
  const records = csvParse(lines.slice(findHeaderIndex(text)).join("\n").trim());
...
function buildDataset({
  ...
  return {
    id: `seafood-imports-${level.id}-${slug}-${granularity}`,
    label: `${granularity === "monthly" ? "Monthly" : "Yearly"} seafood ${level.label} imports - ${country}`,
    country,
    sourceFile,
...
for (const level of levels) {
  const entries = entriesByLevel[level.id];
...
console.log(`Aggregated ${aggregatedTenDigitRows} seafood 10-digit HTS rows to HS8`);
console.log(`Skipped ${skippedRows} seafood rows with unsupported code lengths`);
